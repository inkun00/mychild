...
# ----------------------------------------
# 1) CompletionExecutor 클래스 (기본 코드 기반)
# ----------------------------------------
class CompletionExecutor:
    ...
    def get_response(self, completion_request: dict) -> str:
        ...
        response_text = ""
        # streaming=True로 두어 SSE 형식의 데이터를 실시간으로 읽어들임
        with requests.post(
            ...
            for line in r.iter_lines():
                ...
                if decoded.startswith("data: "):
                    ...
                    try:
                        ...
                        # chunk 구조 예시: {"choices":[{"delta":{"content":"안녕하세요"}}], ...}
                        delta = chunk.get("choices", [])[0].get("delta", {})
                        text = delta.get("content", "")
                        response_text += text
                    except json.JSONDecodeError:
                        # 혹시 순수 텍스트 형태로 넘어올 때 (예외 처리)
...
st.markdown(
    ...
    .reportview-container {
        background-color: #F5F5F7;  /* 연한 회색 계열 */
        font-family: "Apple SD Gothic Neo", "Malgun Gothic", "맑은 고딕", sans-serif;
    }
    /* --- 카카오톡 상단 바 스타일 (유사) --- */
    .header {
        background-color: #FFEB00;  /* 카카오 옐로우 */
        padding: 12px;
...
with chat_container:
    ...
    # 기존 history를 루프 돌며 출력
    for msg in st.session_state.history:
        if msg["role"] == "user":
            st.markdown(f'<div class="bubble-user">{msg["content"]}</div><div style="clear: both;"></div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="bubble-bot">{msg["content"]}</div><div style="clear: both;"></div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

# ----------------------------------------
# 5) 입력 폼 (사용자 메시지)
# ----------------------------------------
# form을 사용해 엔터 혹은 버튼 클릭 시 submit 처리
with st.form(key="input_form"):
    user_input = st.text_input("메시지를 입력하세요...", "", key="input_text", placeholder="메시지를 입력하고 엔터를 누르거나 전송 버튼을 클릭하세요.")
...
if submitted and user_input.strip() != "":
    ...
    # 2) HyperCLOVA용 요청 메시지로 변환 (기본: system + 지금까지 대화 + 신규 user 메시지)
    #    필요한 경우 system 프롬프트를 적절히 수정하세요.
    system_prompt = {
        "role": "system",
    ...
    for msg in st.session_state.history:
        # system 프롬프트는 이미 첫 요소로 들어가 있으므로 user/bot만 추가
    ...
    # 5) 페이지를 다시 렌더링하여 새로 추가된 대화가 보이도록 함
